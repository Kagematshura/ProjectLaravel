...
<form action="{{ url('siswa', @$siswa->id) }}" method="post">
    ...
    Jenis Kelamin :<br>
    <label for="L"><input type="radio" name="jk" value="L" id="L" {{ old('jk', @$siswa->jk) == 'L' ? 'checked' : '' }}>Laki-laki</label><br>
    <label for="P"><input type="radio" name="jk" value="P" id="P" {{ old('jk', @$siswa->jk) == 'P' ? 'checked' : '' }}>Perempuan</label><br>
    Golongan Darah :
